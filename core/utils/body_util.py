...
MANO_JOINT_STDS = np.array([0.02, 0.02, 0.02])
MANO_BONE_STDS = np.array([0.03, 0.06, 0.03])
...
def body_pose_to_body_RTs(jangles, tpose_joints):
    ...
    for i in range(1, total_joints):
        ...
        if USE_MANO:
            Ts[i] = tpose_joints[i, :] - tpose_joints[MANO_PARENT[i], :]
        else:
...
def get_canonical_global_tfms(canonical_joints):
    ...
    # TODO-q: Is it independent of MANO / SMPL?
    for i in range(1, total_bones):
        if USE_MANO:
            translate = canonical_joints[i, :] - canonical_joints[MANO_PARENT[i], :]
            gtfms[i] = gtfms[MANO_PARENT[i]].dot(
...
def approx_gaussian_bone_volumes(
    ...
    for joint_idx in range(0, total_joints):
        ...
        # TODO-q: Is it independent of MANO / SMPL?
        if USE_MANO:
            for bone_idx, parent_idx in MANO_PARENT.items():
                if joint_idx != parent_idx:
                    continue

                S = _std_to_scale_mtx(MANO_BONE_STDS * 2.)
                # if joint_idx in TORSO_JOINTS:
                #     S[0][0] *= 1/1.5
                #     S[2][2] *= 1/1.5

                start_joint = tpose_joints[MANO_PARENT[bone_idx]]
